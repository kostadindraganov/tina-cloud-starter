...
import { SweepstakesConnectionQuery } from "@/tina/__generated__/types"
import { Skeleton } from "@/components/ui/skeleton"
import { Sweepstakes } from "@/store"
import { PaginationInfo } from "../../app/sweepstakes/client-page"
import { SweepstakesCard } from "./sweepstakes-card"
...
type ExtendedSweepstakesConnectionQuery = SweepstakesConnectionQuery & {
  _pagination?: PaginationInfo;
  _allSweepstakes?: Sweepstakes[];
}

interface SweepstakesListProps {
  sweepstakesData: ExtendedSweepstakesConnectionQuery | { sweepstakesConnection: { edges: { node: Sweepstakes }[] } }
  isLoading: boolean
}

// Import the SweepstakesSkeleton directly to avoid module resolution issues
const SweepstakesSkeleton = ({ count = 3 }: { count?: number }) => {
  return (
        ...
        >
          ...
          <div className="w-full md:w-2/4 p-4 md:p-6 border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700">
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 mb-4">
              <Skeleton className="h-12 rounded-lg" /> {/* Visit Sweepstakes */}
...
export function SweepstakesList({ sweepstakesData, isLoading }: SweepstakesListProps) {
  const { theme } = useLayout()

  // If loading, show skeleton
  if (isLoading) {
    return <SweepstakesSkeleton />
  }

  // If no sweepstakes found, show empty state
  if (!sweepstakesData?.sweepstakesConnection?.edges || sweepstakesData.sweepstakesConnection.edges.length === 0) {
    return (
      <div className="text-center p-8 bg-gray-50 dark:bg-gray-800 rounded-lg">
        <p className="text-lg text-gray-600 dark:text-gray-300">No sweepstakes found</p>
      </div>
    )
  }

  // Render sweepstakes list with the new SweepstakesCard component
  return (
    <div className="space-y-8">
      {sweepstakesData.sweepstakesConnection.edges.map((postData) => {
        if (!postData?.node) return null
        const sweepstakes = postData.node

        return (
          <div key={sweepstakes.id} className="mb-8 last:mb-0">
