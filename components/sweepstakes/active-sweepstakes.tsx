...
export default function ActiveSweepstakes() {
  ...
  if (loading) {
    return (
      <div className="w-full">
        <h1 className="text-4xl font-semibold text-gray-700 dark:text-white mb-6">Active Sweepstakes</h1>
  ...
  // Filter sweepstakes to only include active ones
  const activeSweepstakes = data?.sweepstakesConnection?.edges?.filter((postData) => {
  ...
  // If no active sweepstakes, show a message
  if (!activeSweepstakes || activeSweepstakes.length === 0) {
    return (
      <div className="w-full">
        <h1 className="text-4xl font-semibold text-gray-700 dark:text-white mb-6">Active Sweepstakes</h1>
        <p className="text-lg text-gray-600 dark:text-gray-300">
          There are currently no active sweepstakes. Please check back later.
        </p>
      </div>
    );
  }
  ...
  const visibleSweepstakes = activeSweepstakes.slice(0, visibleItems);
  const hasMoreItems = activeSweepstakes.length > visibleItems;
  ...
  return (
    <div className="w-full">
      ...
      <p className="text-lg text-gray-600 dark:text-gray-300 mb-10">
        Check out our <span className="font-medium">on-going sweepstakes</span> that you can enroll in if you comply with the eligibility criteria. These are the only sweepstakes for which you can purchase tickets.
      </p>
