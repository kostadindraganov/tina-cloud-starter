...
interface SweepstakesSupportMethodsProps {
  supportMethods?: SupportMethod[] | null
  className?: string
}

export function SweepstakesSupportMethods({ supportMethods, className }: SweepstakesSupportMethodsProps) {
  ...
  const liveChat = supportMethods.find(method => method?.__typename === 'SweepstakesCustomer_support_methodsLive_chat') as LiveChat | undefined
  const email = supportMethods.find(method => method?.__typename === 'SweepstakesCustomer_support_methodsEmail') as Email | undefined
  const phone = supportMethods.find(method => method?.__typename === 'SweepstakesCustomer_support_methodsPhone') as Phone | undefined
  const availability = supportMethods.find(method => method?.__typename === 'SweepstakesCustomer_support_methodsSupport_availability') as SupportAvailability | undefined
