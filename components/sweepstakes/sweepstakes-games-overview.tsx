...
interface SweepstakesGamesOverviewProps {
  gameCategories?: (GameCategory | null)[] | null
  softwareProviders?: (SoftwareProvider | null)[] | null
  className?: string
}

export function SweepstakesGamesOverview({ gameCategories, softwareProviders, className }: SweepstakesGamesOverviewProps) {
    ...
    <div className={cn(" rounded-md shadow-sm overflow-hidden w-full mx-auto", className)}>
      <div className="flex flex-col md:flex-row w-full gap-8">
        ...
        <div className={`w-full md:w-1/2 ${hasProviderData ? 'border-b md:border-b-0 ' : ''} border-gray-200 dark:border-gray-700  bg-white`}>
          {/* Games Header */}
          <div className=" bg-purple-500 dark:bg-purple-600 p-3 rounded-lg flex items-center justify-between border-b border-purple-500 dark:border-purple-600">
            ...
            <div className="p-3">
              <Tabs defaultValue="grid" className="w-full">
                <TabsList className="mb-3 bg-gray-100 dark:bg-gray-700/50 p-1 rounded-md h-8">
                  <TabsTrigger value="grid" className="text-xs h-6  border-gray-300 dark:border-gray-600 text-purple-600 dark:text-purple-400 data-[state=active]:bg-purple-50 dark:data-[state=active]:bg-purple-900/20">Grid</TabsTrigger>
                  <TabsTrigger value="list" className="text-xs h-6 text-purple-600 dark:text-purple-400 data-[state=active]:bg-purple-50 dark:data-[state=active]:bg-purple-900/20">List</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="grid" className="mt-2">
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                    {displayedGames && displayedGames.map((game, index) => (
                      <div 
                        key={index}
                        className="flex flex-col items-center justify-center p-2 bg-gray-50 dark:bg-gray-800/50 rounded-md border border-gray-100 dark:border-gray-700 hover:border-purple-200 dark:hover:border-purple-800 transition-all duration-200"
                      >
                  ...
                  {hasMoreGames && (
                    <button 
                      ...
                      className="w-full mt-3 text-xs text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 flex items-center justify-center gap-1 py-1"
                    >
                      {showAll ? (
                        <>
                          <span>Show Less</span>
        ...
        {hasProviderData && (
          <div className="w-full md:w-1/2 bg-white">
            {/* Providers Header */}
            <div className="bg-purple-500 dark:bg-purple-600 rounded-lg p-3 flex items-center justify-between border-b border-purple-500 dark:border-purple-600">
              <div className="flex items-center justify-center space-x-2">
            ...
            <div className="p-3">
              <div className="flex flex-wrap gap-1.5">
                {displayedProviders.map((providerName, index) => (
                  <div 
                    key={index}
                    className="px-2 py-1 text-xs bg-gray-50 dark:bg-gray-800/50 text-gray-700 dark:text-gray-300 rounded-md border border-gray-100 dark:border-gray-700 hover:border-purple-200 dark:hover:border-purple-800 transition-all duration-150"
                  >
                    {providerName}
                  </div>
                ))}
