...
export const Section: React.FC<SectionProps> = ({ children, color = '', className = '', style = {} }) => {
  ...
  const sectionColor = {
    ...
    primary: {
      ...
      forest: 'text-white bg-green-900 bg-gradient-to-br from-green-900 to-green-800',
      mint: 'text-white bg-green-800 bg-gradient-to-br from-green-800 to-emerald-600',
      lime: 'text-white bg-green-700 bg-gradient-to-br from-green-700 to-green-700',
      jade: 'text-white bg-teal-900 bg-gradient-to-br from-teal-900 to-teal-800',
      evergreen: 'text-white bg-green-900 bg-gradient-to-r from-green-900 to-emerald-800',
    },
  };
  const sectionColorCss = color === 'primary' ? sectionColor.primary[theme!.color!] : sectionColor[color || 'default'];

  return <section className={`flex-1 relative transition duration-150 ease-out body-font overflow-hidden ${sectionColorCss} ${className}`} style={style}>{children}</section>;
};
