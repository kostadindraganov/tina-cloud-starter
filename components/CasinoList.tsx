...
export default function CasinoList() {
  const { casinos, casinosLoading, casinosPagination, fetchCasinos } = useAppStore()
  ...
  if (casinosLoading) {
    return <div className="py-10 text-center">Loading casinos...</div>
  }
  ...
  return (
    <div>
      ...
      {/* Pagination Controls */}
      {casinosPagination && casinosPagination.totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-8">
          {/* Previous Button */}
          <button
            onClick={() => handlePageChange(casinosPagination.currentPage - 1)}
            disabled={casinosPagination.currentPage === 1}
            className={`px-4 py-2 border rounded ${
              casinosPagination.currentPage === 1
          ...
          {/* Page Numbers */}
          {Array.from({ length: casinosPagination.totalPages }, (_, i) => i + 1).map((page) => (
            <button
              key={page}
              onClick={() => handlePageChange(page)}
              className={`px-4 py-2 border rounded ${
                page === casinosPagination.currentPage
          ...
          <button
            onClick={() => handlePageChange(casinosPagination.currentPage + 1)}
            disabled={casinosPagination.currentPage === casinosPagination.totalPages}
            className={`px-4 py-2 border rounded ${
              casinosPagination.currentPage === casinosPagination.totalPages
                ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                : 'bg-white hover:bg-gray-50 text-blue-500'
            }`}
          >
