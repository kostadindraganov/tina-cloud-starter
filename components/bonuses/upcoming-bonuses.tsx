...
export default function UpcomingBonuses() {
  ...
  if (loading) {
    return (
      <div className="w-full">
        <h1 className="text-4xl font-semibold text-gray-700 dark:text-white mb-6">Upcoming Bonuses</h1>
  ...
  // Filter Bonuses to only include upcoming ones
  const upcomingBonuses = data?.bonusesConnection?.edges?.filter((postData) => {
  ...
  // If no upcoming Bonuses, show a message
  if (!upcomingBonuses || upcomingBonuses.length === 0) {
    return (
      <div className="w-full">
        <h1 className="text-4xl font-semibold text-gray-700 dark:text-white mb-6">Upcoming Bonuses</h1>
        <p className="text-lg text-gray-600 dark:text-gray-300">
          There are currently no upcoming Bonuses. Please check back later.
        </p>
      </div>
    );
  }
  ...
  const visibleBonuses = upcomingBonuses.slice(0, visibleItems);
  const hasMoreItems = upcomingBonuses.length > visibleItems;
  ...
  return (
    <div className="w-full">
      ...
      <p className="text-lg text-gray-600 dark:text-gray-300 mb-10">
        Upcoming Bonuses give you a sneak peek of what's to come. You can check all the details and get ready. If you're not eligible for the upcoming Bonuses, you have the chance to earn extra coins and level up in order to be able to enter once they become active.
      </p>
      ...
      return (
            ...
            >
                  ...
                  <Image
                    ...
                    alt={post.title ? `Upcoming Bonus - ${post.title}` : 'Upcoming Bonus'}
